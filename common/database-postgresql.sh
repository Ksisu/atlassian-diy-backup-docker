...
function prepare_backup_db {
    check_config_var "APPLICATION_BACKUP_DB"
    check_config_var "POSTGRES_USERNAME"
    check_config_var "POSTGRES_HOST"
    check_config_var "POSTGRES_PORT"
    check_config_var "APPLICATION_DB"
}

function backup_db {
    mkdir -p "${APPLICATION_BACKUP_DB}" && rm -r "${APPLICATION_BACKUP_DB}"
    run pg_dump -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} ${PG_PARALLEL} -Fd \
        "${APPLICATION_DB}" ${PG_SNAPSHOT_OPT} -f "${APPLICATION_BACKUP_DB}"
}
...
function prepare_restore_db {
    ...
    check_config_var "POSTGRES_HOST"
    check_config_var "POSTGRES_PORT"
    check_var "APPLICATION_RESTORE_DB"

    if run psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} -d "${APPLICATION_DB}" -c "" 2>/dev/null; then
        local table_count=$(psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} -d "${APPLICATION_DB}" -tqc '\dt' | grep -v "^$" | wc -l)
        if [ "${table_count}" -gt 0 ]; then
            error "Database '${APPLICATION_DB}' already exists and contains ${table_count} tables"
        else
            error "Database '${APPLICATION_DB}' already exists"
        fi
        bail "Cannot restore over existing database '${APPLICATION_DB}', please ensure it does not exist before restoring"
    fi
}

function restore_db {
    run pg_restore -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} ${PG_PARALLEL} \
        -d postgres -C -Fd "${APPLICATION_RESTORE_DB}"
}

function cleanup_incomplete_db_backup {
    info "Cleaning up DB backup created as part of failed/incomplete backup"
    rm -r "${APPLICATION_BACKUP_DB}"
}
...
function promote_db {
    ...
    local is_in_recovery=$(PGPASSWORD="${STANDBY_DATABASE_REPLICATION_USER_PASSWORD}" \
            run psql -U "${STANDBY_DATABASE_REPLICATION_USER_USERNAME}" -d "${APPLICATION_DB}" -tqc "SELECT pg_is_in_recovery()")
...
function get_config_setting {
    ...
    local var_value=$(run psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} \
        -d "${APPLICATION_DB}" -tqc "SHOW ${var_name}")
    echo "${var_value/ }"
}
...
function validate_primary_db {
    ...
    local replication_slot=$(run psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} -d "${APPLICATION_DB}" -tqc \
    ...
    local replication_user=$(run psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} -d "${APPLICATION_DB}" -tqc \
        "\du ${STANDBY_DATABASE_REPLICATION_USER_USERNAME}")

    if [ -z "${replication_user}" ]; then
        run psql -U "${POSTGRES_USERNAME}" -h "${POSTGRES_HOST}" --port=${POSTGRES_PORT} -d "${APPLICATION_DB}" -tqc \
